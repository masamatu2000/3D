...
namespace ObjectManager
{
	...
	/// <summary>
	/// GameObjectの関数が呼べないので代用する
	/// </summary>
	bool IsObjectTag(GameObject* obj, std::string tag);
	...
	{
		std::list<GameObject*> objs = ObjectManager::GetAllObject();

		for (GameObject* node : objs)
		{
			if (not ObjectManager::IsObjectTag(node, tag))
				continue;
...
template<class C> std::list<C*> FindGameObjectsWithTag(std::string tag)
{
	return ObjectManager::FindGameObjectsWithTag<C>(tag);
}
